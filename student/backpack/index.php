...
<div class="row row-cols-4 gx-4 row-cols-sm-1 row-cols-md-3 row-cols-lg-3" id="bp-list">
    ...
    <div class="col bp-item" data-id="<?php echo $row['id'] ?>">
        <div class="card text-dark"
            style="background-image:linear-gradient(rgb(239, 200, 200),rgb(188, 236, 188),rgb(199, 199, 241));">
            <div class="card-header">
                <h5 class='card-title'><?php echo $row['title'] ?></h5>
                <div class="card-tools">
                    <?php if ($row['pinned'] == 1) : ?>
                    <button class="btn bg-none p-0 unpin-item" type="button" data-id="<?php echo $row['id'] ?>"><i
                            class="fa fa-thumbtack"></i></button>
                    <?php else : ?>
                    <button class="btn bg-none p-0 pin-item text-gray opacity-50" type="button"
                        data-id="<?php echo $row['id'] ?>"><i class="fa fa-thumbtack"></i></button>
                    <?php endif; ?>
                </div>
            </div>
            <div class="card-body">
                <div class="container-fluid ">
                    ...
                    <div class="row justify-content-end">
                        <div class="col-auto mt-3">
                            <a href="javascript:void(0)" class="view_bp-item" data-id="<?php echo $row['id'] ?>">View
                                Backpack Item</a>
                        </div>
                    </div>
                </div>
...
$(function() {
    $('.view_bp-item').click(function() {
        uni_modal('Backpack Item View', 'backpack/view_file.php?redirect=true&id=' + $(this).attr(
            'data-id'), "mid-large")
    })
        ...
        success: function(resp) {
            if (resp.status == 'success') {
                ...
                el.find('.view_bp-item').click(function() {
                    uni_modal('Backpack Item View', 'backpack/view_file.php?redirect=true&id=' + $(
