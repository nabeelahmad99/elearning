...
<div class="contriner-fluid" id="file_view">
    ...
    <div class="row justify-content-end">
        ...
        <div class="col-auto mt-3">
            <a class="primary-link mt-2" href="?page=lesson/view_lesson&id=<?php echo $lesson_id ?>">Go to lesson's
                Page</a>
        </div>
        <?php endif; ?>
        <div class="col-auto mt-3">
            <a class="danger-link" href="javascript:void(0)" id="remove_from_backpack">Remove</a>
        </div>
        <div class="col-auto mt-2">
            <button class="default-button w-80" data-dismiss="modal" type="button">Close</button>
        </div>
    </div>
</div>
<script>
$(function() {
    $('#uni_modal').on('hide.bs.modal', function() {
        if ($(this).find('#file_view').length > 0 && !('<?php echo $redirect ?>')) {
            setTimeout(() => {
                uni_modal("My Backpack Items for <?php echo $lesson['title'] ?> lesson",
                    'backpack/per_lesson.php?lesson_id=<?php echo $lesson_id ?>',
                    'mid-large')
            }, 300);
        }
    })
    $('#remove_from_backpack').click(function() {
        _conf("Are you sure to remove this item from your backpack?", "remove_from_backpack", [
            "<?php echo $id ?>"
        ])
    })
})
